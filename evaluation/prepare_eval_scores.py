import pandas as pd
import os

# ECT_DIR = "./evaluation/ect/anticom/recomputed"
# WEAT_DIR = "./evaluation/weat_output/anticom_recomputed"
# ECT_DIR = "./evaluation/ect/original"


DIMENSIONS_SEM = ['sentiment', 'patriotism', 'economic', 'conspiratorial', 'religious', 'racist', 'ethic']
DIMENSIONS_COM = ["sentiment", "politics", "propaganda"]


def prepare_ect_scores():
    dirs = os.listdir(ECT_DIR)
    print(dirs)

    scores = []
    for file in dirs:
        ect_path = ECT_DIR + '/' + file
        scores.append(pd.read_csv(ect_path))

    counter = 0
    for file in dirs:
        print("---------------" + file + "------------------------")
        for dimension in DIMENSIONS_SEM:
            print(dimension)
            for i in range(1, 5):
                print(scores[counter][dimension][i])
        counter += 1


def prepare_weat_scores():
    dirs = os.listdir(WEAT_DIR)
    print(dirs)

    for file in dirs:
        print(file)
        file_path = WEAT_DIR + '/' + file
        file_data = open(file_path, "r")
        data = file_data.readlines()

        result_data = data[1].split(',')
        effect_size = float(result_data[1])
        print(effect_size)


# prepare_ect_scores()
# prepare_weat_scores()


def build_ect_diagram_data():
    dirs = os.listdir(ECT_DIR)
    print(dirs)

    scores = []
    for file in dirs:
        ect_path = ECT_DIR + '/' + file
        scores.append(pd.read_csv(ect_path))

    counter = 0
    scores_aggregated_dim = {}
    scores_aggregated_file = {}
    for file in dirs:
        scores_aggregated = {}
        aggregation = 0
        print("---------------" + file + "------------------------")
        for dimension in DIMENSIONS_SEM:
            ele = 0
            # print(dimension)
            for i in range(1, 5):
                # print(scores[counter][dimension][i])
                ele += float(scores[counter][dimension][i])
            ele = ele / 4
            aggregation += ele
            scores_aggregated[dimension] = ele
        aggregation = aggregation / (len(DIMENSIONS_SEM))
        scores_aggregated_file[file] = aggregation
        scores_aggregated_dim[file] = scores_aggregated

        counter += 1

    return scores_aggregated_dim, scores_aggregated_file


"""
scores_aggregated_dim, scores_aggregated_file = build_ect_diagram_data()
print(scores_aggregated_dim)
print(scores_aggregated_file)

dirs = os.listdir(ECT_DIR)
for file in dirs:
    print(scores_aggregated_file[file])
"""


def build_weat_diagram_data():
    dirs = os.listdir(WEAT_DIR)
    print(dirs)
    scores = {}
    memory = None
    for file in dirs:
        titles = file.title().split("_")
        current_title = titles[0]
        if current_title != memory:
            memory = current_title
            print(memory)

        file_path = WEAT_DIR + '/' + file
        file_data = open(file_path, "r")
        data = file_data.readlines()

        result_data = data[1].split(',')
        effect_size = float(result_data[1])
        scores[file] = effect_size
        # print(file)
        print(effect_size)
    print(scores)
    # print(effect_size)


def build_weat_data():
    dirs = os.listdir(WEAT_DIR)
    print(dirs)
    scores = []
    for file in dirs:
        titles = file.title().split("_")
        try:
            slice = str(titles[1].capitalize()) + str(titles[2].replace(".Txt", ""))
        except:
            slice = str(titles[1])

        specification = "anticommunism_" + titles[0].lower()
        measure = "WEAT"

        file_path = WEAT_DIR + '/' + file
        file_data = open(file_path, "r")
        data = file_data.readlines()

        significance = True
        if "False" in data[0]:
            significance = False

        result_data = data[1].split(',')
        effect_size = float(result_data[1])

        scores.append({"slice": slice, "specification": specification, "measure": measure, "score": effect_size,
                       "significance": significance})
    return scores


# print(build_weat_data())

def build_ect_data():
    dirs = os.listdir(ECT_DIR)
    print(dirs)

    scores = []
    data = []
    for file in dirs:
        ect_path = ECT_DIR + '/' + file
        scores.append(pd.read_csv(ect_path))

    counter = 0
    for file in dirs:
        # print("---------------" + file + "------------------------")
        for dimension in DIMENSIONS_COM:
            # print(dimension)
            result = float(scores[counter][dimension][1])
            specification = "anticommunism_" + dimension
            # specification = "antisemitism_" + dimension
            data.append({"slice": file, "specification": specification, "measure": "ECT", "score": result})
        counter += 1
    return data


# scores = build_ect_data()
scores = build_weat_data()
print(scores)
print()


sentiment = []
political = []
propaganda = []

for score in scores:
    if score["specification"] == "anticommunism_sentiment":
        sentiment.append(score)
    if score["specification"] == "anticommunism_politics":
        political.append(score)
    if score["specification"] == "anticommunism_propaganda":
        propaganda.append(score)

print(str(sentiment[3]) + ",")
print(str(sentiment[4]) + ",")
print(str(sentiment[7]) + ",")
print(str(sentiment[0]) + ",")
print(str(sentiment[5]) + ",")
print(str(sentiment[1]) + ",")
print(str(sentiment[6]) + ",")
print(str(sentiment[2]) + ",")
print('\n')
print(str(political[3]) + ",")
print(str(political[4]) + ",")
print(str(political[7]) + ",")
print(str(political[0]) + ",")
print(str(political[5]) + ",")
print(str(political[1]) + ",")
print(str(political[6]) + ",")
print(str(political[2]) + ",")
print('\n')
print(str(propaganda[3]) + ",")
print(str(propaganda[4]) + ",")
print(str(propaganda[7]) + ",")
print(str(propaganda[0]) + ",")
print(str(propaganda[5]) + ",")
print(str(propaganda[1]) + ",")
print(str(propaganda[6]) + ",")
print(str(propaganda[2]) + ",")

"""
sentiment = []
patriotism = []
economy = []
conspiracy = []
religion = []
racism = []
ethics = []

for score in scores:
    if score["specification"] == "antisemitism_sentiment":
        sentiment.append(score)
    if score["specification"] == "antisemitism_patriotism":
        patriotism.append(score)
    if score["specification"] == "antisemitism_economic":
        economy.append(score)
    if score["specification"] == "antisemitism_conspiratorial":
        conspiracy.append(score)
    if score["specification"] == "antisemitism_religious":
        religion.append(score)
    if score["specification"] == "antisemitism_racist":
        racism.append(score)
    if score["specification"] == "antisemitism_ethic":
        ethics.append(score)

print(str(sentiment[3]) + ",")
print(str(sentiment[4]) + ",")
print(str(sentiment[7]) + ",")
print(str(sentiment[0]) + ",")
print(str(sentiment[5]) + ",")
print(str(sentiment[1]) + ",")
print(str(sentiment[6]) + ",")
print(str(sentiment[2]) + ",")
print('\n')
print(str(patriotism[3]) + ",")
print(str(patriotism[4]) + ",")
print(str(patriotism[7]) + ",")
print(str(patriotism[0]) + ",")
print(str(patriotism[5]) + ",")
print(str(patriotism[1]) + ",")
print(str(patriotism[6]) + ",")
print(str(patriotism[2]) + ",")
print('\n')
print(str(economy[3]) + ",")
print(str(economy[4]) + ",")
print(str(economy[7]) + ",")
print(str(economy[0]) + ",")
print(str(economy[5]) + ",")
print(str(economy[1]) + ",")
print(str(economy[6]) + ",")
print(str(economy[2]) + ",")
print('\n')
print(str(conspiracy[3]) + ",")
print(str(conspiracy[4]) + ",")
print(str(conspiracy[7]) + ",")
print(str(conspiracy[0]) + ",")
print(str(conspiracy[5]) + ",")
print(str(conspiracy[1]) + ",")
print(str(conspiracy[6]) + ",")
print(str(conspiracy[2]) + ",")
print('\n')
print(str(religion[3]) + ",")
print(str(religion[4]) + ",")
print(str(religion[7]) + ",")
print(str(religion[0]) + ",")
print(str(religion[5]) + ",")
print(str(religion[1]) + ",")
print(str(religion[6]) + ",")
print(str(religion[2]) + ",")
print('\n')
print(str(racism[3]) + ",")
print(str(racism[4]) + ",")
print(str(racism[7]) + ",")
print(str(racism[0]) + ",")
print(str(racism[5]) + ",")
print(str(racism[1]) + ",")
print(str(racism[6]) + ",")
print(str(racism[2]) + ",")
print('\n')
print(str(ethics[3]) + ",")
print(str(ethics[4]) + ",")
print(str(ethics[7]) + ",")
print(str(ethics[0]) + ",")
print(str(ethics[5]) + ",")
print(str(ethics[1]) + ",")
print(str(ethics[6]) + ",")
print(str(ethics[2]) + ",")
"""
