...
from bias_specifications import antisemitic_streams, anticommunism_streams
    ...
    def plot_onto_bias_direction(self, projections_1, projections_2, style='ggplot', output_file=''):
        ...
        with plt.style.context(style):
            ...
            for i in range(0, len(projections_1), 2):
                ...
                ax.annotate(k, xy=(v, 0), xytext=(v, 0.025), rotation=90, ma='left', fontsize='xx-small',
            ...
            for i in range(0, len(projections_2), 2):
                ...
                ax.annotate(k, xy=(v, 0), xytext=(v, -0.05), rotation=90, ma='left', fontsize='xx-small',
                            fontstretch='ultra-condensed',
                            arrowprops=dict(facecolor='blue', shrink=0.02, alpha=0.15, width=1, headwidth=2))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compute subspace projections onto ethno-religious bias subspace - plot and/or compute t-test between them")
    ...
    parser.add_argument("--sem_domain", nargs='?',
                        choices=['sentiment', 'patriotism', 'economic', 'conspiratorial', 'racist', 'religious',
                                 'ethic'], help='Which semantic domain to test in WEAT', required=True)
    ...
    parser.add_argument("--plot_projections", type=boolean_string, default=True,
                        help="Whether to plot subspace projections", required=True)
    parser.add_argument("--plot_pca", type=boolean_string, default=False, help="Whether to plot subspace projections",
                        required=True)
    parser.add_argument("--t_test", type=boolean_string, default=True,
                        help="Whether to compute t-test for a semantic domain ", required=True)
