...
from environment import MODULES_DIR

logging.basicConfig(level=logging.INFO, format='%(asctime)s :: %(levelname)s :: %(message)s')

sys.path.append(os.path.abspath(os.path.join(MODULES_DIR, 'CharSplit')))
sys.path.append(os.path.abspath(os.path.join(MODULES_DIR, 'germalemmaplusplus')))
from modules.CharSplit import char_split
from modules.germalemmaplusplus.lemmatizer import GermaLemmaPlusPlus, Token, Sentence
...
def remove_noisy_digits(doc: List) -> List:
    ...
    temp = [re.sub(r'\b(?P<quote>[A-Za-zßäöüÄÖÜ]+)\d(?P<name>[A-Za-zßäöüÄÖÜ]*)\b', '\g<quote> \g<name>', line) for line
            in temp]
    return temp
...
def remove_german_chainwords(sentence: str) -> str:
    ...
    regex.append(
        r"\s([A-ZÄÖÜ][a-zäöüß]+)[\s]?-[\s]?([A-ZÄÖÜ][a-zäöüß]+)-[\s]?(?:und|oder|als auch|sowie|wie|bzw|&|,)+[\s]([A-ZÄÖÜ][a-zäöüß]+)")
    ...
    regex.append(
        r"\s([A-ZÄÖÜ][a-zäöüß]+)[' ']-?[' ']?(?:und|oder|als auch|wie|sowie|bzw)+[' ']-[' ']?([A-Za-zäöüßÄÖÜß]+)")
